...
var details = document.querySelector(".my-details");
var lifeCode = document.querySelectorAll(
  ".life-code>pre em.parent,em.children,em.parent-children"
);
let quoteCode = document.querySelector(".life-code>pre");
let contact = document.querySelector(".contact-me");
console.log(lifeCode);
const colors = [
...
let background = "#131716";
let newCoordinates = [{ x: -350, y: 250 }, { x: 100, y: 100 }, { x: 0, y: 0 }];
let counter = 0;
const maxOptions = newCoordinates.length - 1;
let currentOption = 0;
...
function Particle(x, y, radius, color) {
  let xDistance = randomIntFromRange(50, 320);
  let yDistance = randomIntFromRange(60, 65);
  this.x = x;
  this.y = y;
  this.dx = 1;
  this.dy = 1;
...
Particle.prototype.draw = function() {
  c.beginPath();
  c.arc(this.x, this.y, this.radius.size, 0, Math.PI * 2, false);
...
Particle.prototype.update = function() {
  ...
  if (
    ...
    if (this.radius.size < this.radius.max) this.radius.size += 1;
...
function animate() {
  ...
  if (counter % 125 === 0) {
    particles.forEach(particle => {
      let newX = particle.originalDistance.x + newCoordinates[currentOption].x;
      let newY = particle.originalDistance.y + newCoordinates[currentOption].y;

      switch (currentOption) {
        case 1:
          newY = newY * Math.random();

          break;
      }

      TweenLite.to(particle.distanceFromCenter, 1.5, {
        x: newX,
        y: newY,
        ease: Expo.easeOut
      });
...
const adjust = document.querySelector(".mode");
adjust.addEventListener("click", () => {
  if (!adjust.className.includes("fa-adjust")) {
    canvas.style.display = "block";
    adjust.classList.remove("fa-moon");
    adjust.classList.add("fa-adjust");
    details.classList.remove("navyBlue");
    contact.classList.remove("navyBlue");
    lifeCode.forEach(n => {
      n.classList.remove("navyBlue");
    });
  } else {
    canvas.style.display = "none";
    body.style.backgroundColor = "#F2EFEF";
    head.style.background = "#131615";
    details.classList.add("navyBlue");
    contact.classList.add("navyBlue");
    lifeCode.classList.add('navyBlue');
    // lifeCode.forEach(n => {
    //   n.classList.add("navyBlue");
    // });
    adjust.classList.remove("fa-adjust");
    adjust.classList.add("fa-moon");
  }
});
